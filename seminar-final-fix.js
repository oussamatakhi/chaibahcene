...
function restoreTeacherFields(f){
 ...
 if(!select){
   const box=document.createElement('label');box.className='full';box.innerHTML='<span>الأساتذة المعنيون بالندوة</span><select id="seminarTeachers" name="teacher_ids" multiple size="8"></select>';f.querySelector('button[type=submit]').before(box);select=box.querySelector('select');
 }
 ...
 let oldFac=[...f.querySelectorAll('input[name="facilitators"],textarea[name="facilitators"],input[name="moderators"],textarea[name="moderators"]')];
 oldFac.forEach(x=>x.closest('label')?.remove()||x.remove());
 const fac=document.createElement('label');fac.className='full';fac.innerHTML='<span>المؤطرون</span><input type="text" name="facilitators" placeholder="أدخل أسماء المؤطرين">';
 select.closest('label')?.after(fac) || select.parentElement?.after(fac);
 return select;
}
function addStyles(){if($('seminarTeacherStyle'))return;const s=document.createElement('style');s.id='seminarTeacherStyle';s.textContent='.seminar-extra-fields,.seminar-choice-options,.seminar-teachers-list,.seminar-teacher-choice{display:none!important}#seminarTeachers{width:100%;min-height:180px;padding:11px 12px;border:1px solid #dbe2ea;border-radius:9px;font-family:Cairo,sans-serif;background:#fff}#seminarTeachers option{padding:7px 6px}.seminar-teacher-institution{color:#64748b}';document.head.appendChild(s)}
async function save(f,select){const data=Object.fromEntries(new FormData(f).entries());const ids=[...select.selectedOptions].map(o=>o.value);const client=C();if(!client){alert('تعذر الاتصال بقاعدة البيانات.');return}const payload={title:data.title,seminar_date:data.seminar_date,start_time:data.start_time||null,end_time:data.end_time||null,location:data.location||null,theme:data.theme||null,objectives:data.objectives||null,status:'مبرمجة'};if(data.facilitators!==undefined)payload.facilitators=data.facilitators||null;const r=await client.from('seminars').insert(payload).select().single();if(r.error){alert('تعذر تسجيل الندوة: '+r.error.message);return}if(ids.length){const a=await client.from('seminar_attendance').insert(ids.map(id=>({seminar_id:r.data.id,teacher_id:id,present:true})));if(a.error){alert('تم تسجيل الندوة لكن تعذر ربط الأساتذة: '+a.error.message);return}}alert('تم تسجيل الندوة بنجاح');f.reset();restoreTeacherFields(f);window.closeModal?.('seminarModal');await window.loadAll?.()}
