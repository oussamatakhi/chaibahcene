...
function restoreTeacherFields(f){
 ...
 if(!select){const box=document.createElement('label');box.className='full';box.innerHTML='<span>الأساتذة المعنيون بالندوة</span><select id="seminarTeachers" name="teacher_ids" multiple size="8"></select><button type="button" class="seminar-select-all">تحديد الكل</button>';f.querySelector('button[type=submit]').before(box);select=box.querySelector('select')}else if(!select.closest('label')?.querySelector('.seminar-select-all')){const b=document.createElement('button');b.type='button';b.className='seminar-select-all';b.textContent='تحديد الكل';select.closest('label')?.appendChild(b)}
 ...
 const allBtn=select.closest('label')?.querySelector('.seminar-select-all');if(allBtn&&!allBtn.dataset.bound){allBtn.dataset.bound='1';allBtn.addEventListener('click',()=>[...select.options].forEach(o=>o.selected=true))}
 ...
 const fac=document.createElement('label');fac.className='full';fac.innerHTML='<span>المؤطرون</span><input type="text" id="seminarFacilitators" name="facilitators" autocomplete="off" placeholder="أدخل أسماء المؤطرين">';select.closest('label')?.after(fac)||select.parentElement?.after(fac);
 return select;
}
