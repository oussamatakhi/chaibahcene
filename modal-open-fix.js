...
(function(){
  'use strict';
  function show(id){const el=document.getElementById(id);if(!el)return false;el.hidden=false;el.removeAttribute('hidden');el.style.display='grid';document.body.classList.add('modal-open');return true}
  function hide(id){const el=document.getElementById(id);if(!el)return false;el.hidden=true;el.setAttribute('hidden','');el.style.display='none';if(!document.querySelector('.modal:not([hidden])'))document.body.classList.remove('modal-open');return true}
